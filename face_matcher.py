...
class FaceMatcher:
    ...
    def find_faces_batch(self, input_list, do_log=False, track_progress=200, match_model="cnn"):
        ...
        print("All completed. Average processing time of", (finish_time / len(input_list)), "seconds per image")
    ...
    def create_template(self, image_path, location): # This is like a single enrollment
        ...
        template = fr.face_encodings(face_image, known_face_locations=[location])
        return template
    ...
    def enroll_batch(self, enr_list):
        ...
        for item in enr_list:
            enrollment_dict[item[0]] = self.create_template(item[1][0], item[1][1])
            if(item[0] % 100 == 0):
                print("Enrolled...", item[0])
        print("Finished enrolling", len(enr_list), "in", int(time.time() - start), "seconds")
    ...
    def cross_match_pairs(self, enrollments, pairs, csv_output):
        ...
        for count, pair in enumerate(pairs):
            score = self.generate_score(enrollments[pair[0]], enrollments[pair[1]], mate=True, logging=False, new_threshold=False)
            scores.append(str(pair[0]) + "," + str(pair[1]) + "," + str(score))
            if(count % 5000 == 0):
                print(count, "matches completed")
        print("Finished matching pairs in", int(time.time() - start), "seconds")
